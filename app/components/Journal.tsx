  ...
  const toggleSound = () => {
    if (isMuted) {
      try {
        ...
        osc.type = "sine";
        osc.frequency.setValueAtTime(216, ctx.currentTime); // 216Hz harmonic soothing note
        gain.gain.setValueAtTime(0.08, ctx.currentTime); // Rich, audible ambient volume
        ...
        <div className="hidden md:flex items-center gap-4">
          <button
            ...
            className={`p-2.5 rounded-full glass-card transition-all ${
              !isMuted ? "text-[#6F8F78] border-[#6F8F78] shadow-lg shadow-[#6F8F78]/20" : "text-[#8E9AAF] hover:text-[#F4F0E8]"
            }`}
            title={isMuted ? "Unmute Calming Ambient Sound" : "Mute Ambient Sound"}
          >
